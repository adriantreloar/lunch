...
class DimensionDataFrameMerger(Transformer):
    ...
    def __init__(self, source_df: pd.DataFrame, compare_df: pd.DataFrame, dimension_model: dict) -> pd.DataFrame:
    ...
    def merge(self, key: list[str]) -> pd.DataFrame:
        return _merge(source_df=self._source_dataframe, compare_df=self._compare_dataframe, key=key)


def _merge(source_df: pd.DataFrame, compare_df: pd.DataFrame,key: list[str]) -> pd.DataFrame:
