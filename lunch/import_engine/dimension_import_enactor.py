...
class DimensionImportEnactor(Conductor):
    ...
    async def enact_plan(self,
        ...
        return await _enact_plan(import_plan=import_plan,
                                 data=data,
...
async def _enact_plan(self,
    ...
    read_columns, read_index = await dimension_data_store.get(dimension_id=import_plan["dimension_id"], filter=import_plan.get("read_filter"))

    # We are making the
    compare_df = await DimensionDataFrameTransformer.make_dataframe(columns=read_columns, index=read_index)

    merged_df = await DimensionDataFrameTransformer.merge(source_df=data, compare_df=compare_df, key=import_plan["merge_key"])
