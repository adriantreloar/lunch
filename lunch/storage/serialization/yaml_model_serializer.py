...
class YamlModelSerializer(ModelSerializer):
    ...
    async def get_dimension_id(self, name: str, version: Version) -> int:
        return await _get_dimension_id(
            name=name, version=version, persistor=self._persistor
        )
    ...
    async def get_fact_index(self, version: Version) -> dict:
        return await _get_fact_index(version=version, persistor=self._persistor)

    async def put_fact_index(self, fact_index: dict, version: Version):
        return await _put_fact_index(
            fact_index=fact_index, version=version, persistor=self._persistor
        )
...
async def _get_fact_index(version: Version, persistor: LocalFileModelPersistor):
    if not version.model_version:
        return {}

    try:
        with persistor.open_fact_index_file_read(
            version=version.model_version
        ) as stream:
            d = yaml.safe_load(stream)
    except FileNotFoundError:
...
async def _put_fact_index(
    fact_index: dict, version: Version, persistor: LocalFileModelPersistor
):
    with persistor.open_fact_index_file_write(version=version.model_version) as stream:
        yaml.safe_dump(fact_index, stream)
