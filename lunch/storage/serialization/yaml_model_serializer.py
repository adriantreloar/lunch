...
async def _get_dimension(dimension: dict, version: Version, persistor: LocalFileModelPersistor):
    with await persistor.open_dimension_file_read(dimension["name"], version.model_version) as stream:
        dict = await yaml.load(stream)
    return dict


async def _put_dimension(dimension: dict, version: Version, persistor: LocalFileModelPersistor):

    with await persistor.open_dimension_file_write(dimension["name"], version.model_version) as stream:
        await yaml.dump(dict, stream)
