...
class FactIndexTransformer(Transformer):
    ...
    def add_fact_name_to_index(
        ...
        try:
            ...
            max_id = FactIndexTransformer.get_max_id(
                fact_index_dict=fact_index_dict
            )
    ...
    @staticmethod
    def update_fact_version_index(index:dict[int,int], write_version: Version, changed_ids: list[int]) -> dict[int,int]:
        copy_index = index.copy()
        copy_index.update({id_: write_version.model_version for id_ in changed_ids})
        return copy_index
    @staticmethod
    def update_fact_name_index(index:dict[str:int], changed_names_index: dict[str:int]) -> dict[str,int]:
        copy_index = index.copy()
