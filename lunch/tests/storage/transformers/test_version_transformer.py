...
def test_start_new_write_version_on_empty():
    ...
    new_versions_dict, write_version = VersionsTransformer.start_new_write_version_in_versions(versions_dict={}, read_version=NOUGHT, model=True, reference=False, cube=False, operations=False, website=False)

    NEW_VERSION = Version(version=1, model_version=1, reference_data_version=0, cube_data_version=0, operations_version=0,website_version=0)

    assert write_version == NEW_VERSION
    assert new_versions_dict == {"versions": {1:{"version":version_to_dict(version=NEW_VERSION), 'committed': False,
                  'readers': 0,
                  'status': 'writing'}}}
