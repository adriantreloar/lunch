...
async def insert_fact_data():
    ...
    # Create some fact data to append to f_sales
    ...
    # Create the map between data columns and where they will go in the fact star schema model
    ...
    source_metadata = TableMetadata(
        column_names=list(df_data.columns), column_types=list(df_data.dtypes), length=df_data.shape[0]
    )

    fact_data_persistor = LocalFileColumnarFactDataPersistor(directory=_EXAMPLE_OUTPUT / "fact")
    fact_data_cache = NullFactDataCache()
    fact_data_serializer = ColumnarFactDataSerializer(persistor=fact_data_persistor)

    fact_data_storage = FactDataStore(serializer=fact_data_serializer, cache=fact_data_cache)
    ...
    async with version_manager.read_version() as read_version:
        ...
        read_version_sales_star_schema = await model_manager.get_star_schema_model_by_fact_name(
            name="Sales", version=read_version
        )

        async with version_manager.write_cube_data_version(read_version=read_version) as write_version:

            write_version_sales_star_schema = await model_manager.get_star_schema_model_by_fact_name(
                name="Sales", version=write_version
            )
