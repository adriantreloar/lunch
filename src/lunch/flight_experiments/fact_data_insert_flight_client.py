...
async def insert_fact_data_example():
    ...
    data = [
        {"department_thing": f"A Thing {j}", "thing 2": f"Time {i}", "sales value": j + 0.10}
    ...
    column_mapping = [
        {"source": ["department_thing"], "target": ["Department", "thing1"]},
        {
            "source": ["thing 2"],
            "target": ["Time", "thing2"],
        },
    ...
    async with version_manager.read_version() as read_version:
        async with version_manager.write_reference_data_version(read_version=read_version) as write_version:
            ...
            writer, reader = client.do_put(
                descriptor=upload_descriptor, schema=pa.schema(fields=[]), options=call_options
            )
