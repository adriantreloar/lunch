...
class DimensionDataFrameTransformer(Transformer):
    @staticmethod
    def make_dataframe(columns: Mapping[int, Iterable], dtypes: Mapping[int, np.dtype]) -> pd.DataFrame:
        """
        ...
        series = {
            column_id: pd.Series(data=iterable, dtype=dtypes.get(column_id, np.dtype("object")))
