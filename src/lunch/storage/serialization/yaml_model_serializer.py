...
class YamlModelSerializer(ModelSerializer):
    ...
    async def put_dimension_name_index(self, index_: dict, version: Version):
        return await _put_dimension_name_index(index_=index_, version=version, persistor=self._persistor)

    async def put_dimension_version_index(self, index_: dict[int, int], version: Version):
        return await _put_dimension_version_index(index_=index_, version=version, persistor=self._persistor)
    ...
    async def put_fact_name_index(self, fact_index: dict, version: Version):
        return await _put_fact_name_index(index_=fact_index, version=version, persistor=self._persistor)
    ...
    async def put_fact_version_index(self, fact_index: dict, version: Version):
        return await _put_fact_version_index(index_=fact_index, version=version, persistor=self._persistor)
...
async def _get_dimension(id_: int, model_version: int, persistor: LocalFileModelPersistor):
...
async def _put_dimensions(dimensions: list[dict], model_version: int, persistor: LocalFileModelPersistor):
    # TODO - this can be done in parallel perhaps
    for dimension in dimensions:
        with persistor.open_dimension_file_write(id_=dimension["id_"], version=model_version) as stream:
...
async def _get_dimension_name_index(version: Version, persistor: LocalFileModelPersistor):
    ...
    try:
        with persistor.open_dimension_name_index_file_read(version=version.model_version) as stream:
            d = yaml.safe_load(stream)
...
async def _put_dimension_name_index(index_: dict, version: Version, persistor: LocalFileModelPersistor):
    with persistor.open_dimension_name_index_file_write(version=version.model_version) as stream:
        yaml.safe_dump(index_, stream)


async def _put_dimension_version_index(index_: dict, version: Version, persistor: LocalFileModelPersistor):
    with persistor.open_dimension_version_index_file_write(version=version.model_version) as stream:
        yaml.safe_dump(index_, stream)
...
async def _get_dimension_version_index(version: Version, persistor: LocalFileModelPersistor):
    ...
    try:
        with persistor.open_dimension_version_index_file_read(version=version.model_version) as stream:
            d = yaml.safe_load(stream)
    except FileNotFoundError:
        raise KeyError(version)
    return d
...
async def _get_fact(id_: int, version: Version, persistor: LocalFileModelPersistor) -> Fact:
    ...
    with persistor.open_fact_file_read(id_=id_, version=version.model_version) as stream:
        d = yaml.safe_load(stream)
    return Fact(**d)


async def _put_fact(fact: dict, version: Version, persistor: LocalFileModelPersistor):
    with persistor.open_fact_file_write(id_=fact["id_"], version=version.model_version) as stream:
        yaml.safe_dump(fact, stream)


async def _put_facts(facts: list[Fact], version: Version, persistor: LocalFileModelPersistor):
    # TODO - this could be done in parallel perhaps?
    for fact in facts:
        with persistor.open_fact_file_write(id_=fact.fact_id, version=version.model_version) as stream:
...
async def _get_fact_version_index(version: Version, persistor: LocalFileModelPersistor):
    ...
    try:
        with persistor.open_fact_version_index_file_read(version=version.model_version) as stream:
            d = yaml.safe_load(stream)
    except FileNotFoundError:
        raise KeyError(version)
    return d
...
async def _put_fact_version_index(index_: dict, version: Version, persistor: LocalFileModelPersistor):
...
async def _get_fact_name_index(version: Version, persistor: LocalFileModelPersistor):
    ...
    try:
        with persistor.open_fact_name_index_file_read(version=version.model_version) as stream:
            d = yaml.safe_load(stream)
    except FileNotFoundError:
        raise KeyError(version)
    return d
...
async def _put_fact_name_index(index_: dict, version: Version, persistor: LocalFileModelPersistor):
