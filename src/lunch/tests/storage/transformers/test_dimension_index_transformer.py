...
def test_add_name_to_empty_names_dict():
    ...
    result = transformer.update_dimension_name_index(index_={}, changed_names_index={"Foo": 1})
    assert result == {"Foo": 1}


def test_update_name_in_dict():
    transformer = DimensionModelIndexTransformer()
    result = transformer.update_dimension_name_index(index_={"Foo": 1}, changed_names_index={"Bar": 1})
    assert result == {"Bar": 1}


def test_add_entry_to_empty_versions_dict():
    transformer = DimensionModelIndexTransformer()
    result = transformer.update_dimension_version_index(index_={}, write_version=TEN_NINE_EIGHT, changed_ids=[1, 2, 3])
